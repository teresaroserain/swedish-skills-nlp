...
INPUT_JSON_CANDIDATES = [
    "data/processed/merged/dataset_clean_merged_5skills.jsonl",
    "data/processed/merged/dataset_clean_merged_5skills.json",
]
SPLIT_MANIFEST_CANDIDATES = [
    "data/splits/manifests/merged_all_skills_split_manifest_seed13.csv",
]
EXPECTED_INPUT_DIRECTORY = "merged"


TRAIN_TARGET_FIELD = "all_skills"
TEST_GOLD_LIST_FIELD = "all_skills_list"
RUN_NAME = "bart_merged_all_skills"
...
VALID_OUT_BASE = f"{RUN_NAME}_validation_seed"
...
def run_one_seed(seed: int) -> Dict[str, float]:
    ...
    if DO_HPO:
        ...
        pd.DataFrame(trial_logs).to_csv(
            f"{RUN_NAME}_hpo_trials_seed{seed}.csv",
            index=False,
            encoding="utf-8-sig",
        )
